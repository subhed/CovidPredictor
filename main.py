...
def gbc_fun():
    if request.method == "POST":
        ...
        file = open('model/model_GBC.pkl', 'rb')
...
@app.route('/gbr', methods=["GET", "POST"])
def gbr_fun():
    if request.method == "POST":
        myDict = request.form
        fever = float(myDict['fever'])
        ...
        file = open('model/model_GBR.pkl', 'rb')
        ...
        ypred = gbr.predict(O_test_x)
        # infProb =mlp.predict_proba([inputFeatures])[0][1]

        print("Prediction :", ypred)
        acc=gbr.score(O_test_x, O_test_y) 
        print("Accuracy: ", acc)
        # print(gbc.predict_proba(O_test_x))
        ...
        return render_template('show_gbr.html', inf= (ans))
    return render_template('index_gbr.html')
...
def mlp_fun():
    if request.method == "POST":
        ...
        file = open('model/model_MLP.pkl', 'rb')
        ...
        return render_template('show_mlp.html', inf= round(inff*100))
    return render_template('index_mlp.html')
    
   # return 'Hello, World!' + str(infProb)



@app.route('/mlpr', methods=["GET", "POST"])
def mlpr_fun():
    if request.method == "POST":
        myDict = request.form
        fever = float(myDict['fever'])
        ...
        file = open('model/model_MLPR.pkl', 'rb')
        ...
        return render_template('show_mlpr.html', inf= round(inff*100))
    return render_template('index_mlpr.html')
